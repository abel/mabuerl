...
%%%
%%% @end
%%% Created : 04. 十二月 2014 11:23
%%%-------------------------------------------------------------------
-module(mabu_listener).
-author("yanghaicheng").
...
%% @end
%%----------------------------------------------------------------------
init([Port, Module]) when is_integer(Port), is_atom(Module) ->
...
handle_info({inet_async, ListSock, Ref, {ok, CliSocket}},
  ...
  try
    ...
    {ok, Pid} = supervisor:start_child(mabu_client_sup, [Module]),
    gen_tcp:controlling_process(CliSocket, Pid),
    mabu_client:start_client(Pid, CliSocket),
